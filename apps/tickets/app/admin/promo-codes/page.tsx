...
type PromoRow = {
  id: string; code: string; partner_name: string; ambassador_hash: string | null; discount_type: string;
  discount_percent: number | null; discount_fixed_grosze: number | null; commission_grosze: number; scope: string; event_id: string | null;
...
export default async function PromoCodesPage() {
  ...
  const [{ data: events, error: eventsError }, { data: promos, error: promosError }, { data: orders, error: ordersError }, { data: visits, error: visitsError }] = await Promise.all([
    ...
    supabase.from("promo_codes").select("id,code,partner_name,ambassador_hash,discount_type,discount_percent,discount_fixed_grosze,commission_grosze,scope,event_id,landing_event_id,is_active,max_redemptions,starts_at,ends_at").order("created_at", { ascending: false }),
    supabase.from("orders").select("promo_code_id,status,quantity,amount_grosze,promo_discount_grosze,ambassador_commission_grosze").not("promo_code_id", "is", null),
  ...
  const stats = new Map<string, { visits: number; paidOrders: number; tickets: number; paidRevenue: number; discount: number; commission: number }>();
  for (const visit of visits ?? []) {
    const id = visit.promo_code_id as string;
    const s = stats.get(id) ?? { visits: 0, paidOrders: 0, tickets: 0, paidRevenue: 0, discount: 0, commission: 0 };
  ...
  for (const order of orders ?? []) {
    ...
    if (order.status === "paid") {
      s.paidOrders += 1;
      s.tickets += (order as { quantity?: number }).quantity ?? 0;
      s.paidRevenue += order.amount_grosze as number;
      s.discount += (order as { promo_discount_grosze?: number }).promo_discount_grosze ?? 0;
      s.commission += (order as { ambassador_commission_grosze?: number }).ambassador_commission_grosze ?? 0;
    }
    stats.set(id, s);
  }
  ...
  return <div className="space-y-8">
    ...
    <div className="overflow-x-auto rounded-2xl border border-poet-gold/15"><table className="min-w-full text-left text-sm"><thead className="bg-zinc-900/80 text-xs uppercase text-zinc-500"><tr><th className="px-3 py-3">Код / партнёр</th><th className="px-3 py-3">Правило</th><th className="px-3 py-3">Ссылка</th><th className="px-3 py-3">Переходы</th><th className="px-3 py-3">Продажи</th><th className="px-3 py-3">Выручка</th><th className="px-3 py-3">Комиссия</th><th className="px-3 py-3">Период</th></tr></thead><tbody className="divide-y divide-poet-gold/10">{(promos as PromoRow[] ?? []).map((promo) => {
      ...
      const discountLabel = promo.discount_type === "fixed" ? `${formatPlnFromGrosze(promo.discount_fixed_grosze ?? 0)} / билет` : `${promo.discount_percent}%`;
      return <tr key={promo.id} className="bg-zinc-950/40 align-top"><td className="px-3 py-3"><code className="font-semibold text-poet-gold-bright">{promo.code}</code><div className="mt-1 text-xs text-zinc-500">{promo.partner_name}</div>{promo.ambassador_hash ? <a href={`/ru/ambassador/${promo.ambassador_hash}`} className="mt-1 block text-xs text-violet-300 underline">кабинет ↗</a> : null}</td><td className="px-3 py-3 text-zinc-300">−{discountLabel} · {promo.scope === "all" ? "все" : promo.scope === "special" ? "special" : "одно событие"}{promo.max_redemptions ? <div className="text-xs text-zinc-500">лимит: {promo.max_redemptions}</div> : null}</td><td className="px-3 py-3 text-xs">{href ? <a className="break-all text-poet-gold underline" href={href}>{href}</a> : <span className="text-zinc-600">Выберите событие для ссылки</span>}</td><td className="px-3 py-3 text-zinc-300">{s.visits}</td><td className="px-3 py-3 text-zinc-300">{s.paidOrders}<div className="text-xs text-zinc-500">билетов {s.tickets}</div></td><td className="px-3 py-3 text-zinc-300">{formatPlnFromGrosze(s.paidRevenue)}<div className="text-xs text-zinc-500">скидки {formatPlnFromGrosze(s.discount)}</div></td><td className="px-3 py-3 text-zinc-300">{formatPlnFromGrosze(s.commission)}<div className="text-xs text-zinc-500">{formatPlnFromGrosze(promo.commission_grosze)} / билет</div></td><td className="px-3 py-3 text-xs text-zinc-500">{promo.starts_at ? `с ${formatPlDateTime(promo.starts_at)}` : "сразу"}<br />{promo.ends_at ? `до ${formatPlDateTime(promo.ends_at)}` : "без конца"}</td></tr>;
    })}</tbody></table></div>
  </div>;
}
