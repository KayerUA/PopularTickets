...
const ADMIN_EVENT_SELECT_BASE =
  "id,slug,title,description,title_pl,description_pl,title_uk,description_uk,image_url,image_focal_x,image_focal_y,venue,starts_at,price_grosze,day_of_event_price_grosze,total_tickets,visibility,listing_kind,discount_periods,event_language" as const;
...
export default async function EditEventPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (r1.error) {
    if (isEventsPoetCourseIdUnavailable(r1.error.message)) {
      ...
      const legacySel =
        "id,slug,title,description,title_pl,description_pl,title_uk,description_uk,image_url,image_focal_x,image_focal_y,venue,starts_at,price_grosze,day_of_event_price_grosze,total_tickets,visibility,listing_kind,discount_periods,poet_course_id" as const;
      ...
      const legacySel =
        "id,slug,title,description,image_url,image_focal_x,image_focal_y,venue,starts_at,price_grosze,day_of_event_price_grosze,total_tickets,visibility,listing_kind,discount_periods,poet_course_id" as const;
      const r2 = await supabase.from("events").select(legacySel).eq("id", id).maybeSingle();
      if (r2.error) notFound();
  ...
  const row: AdminEventRow = {
    ...
    listing_kind: (event.listing_kind as "performance" | "trial" | "special") ?? "performance",
