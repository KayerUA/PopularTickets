...
Команды: `/start` — главное меню; `/cancel` — отменить текущий черновик до создания; `/broadcast` — подготовить произвольный пост к рассылке.
...
**Афиша события** — после публикации на сайте бот предлагает выбрать адресата: **🌐 все подключённые группы** или **⭐ мастер‑группа POPULAR IMPRO**. Мастер‑группа закреплена в коде как уже подключённая группа; при добавлении новых групп она остаётся быстрым вариантом для основного канала.

**Произвольный пост** — `/broadcast` в личке бота (владелец или редактор):
1. Нажмите «📣 Отправить сообщение» в главном меню или `/broadcast` → пришлите текст, фото, видео или альбом → выберите все группы или мастер‑группу.
2. Или ответьте `/broadcast` на уже отправленное сообщение.
3. `/post` — то же самое. `/cancel` — выйти из режима рассылки.

**Фото к готовому событию** — пришлите в личный чат одно фото без текста. Если есть будущие события без обложки, бот предложит их кнопками и загрузит фото только в выбранное событие. При активном черновике фото по-прежнему добавляется к нему.

**Предстоящие события** — `/events` (или `/upcoming`, `/афиша`):
- Список будущих событий на сайте (`published` и скрытые `unlisted`).
- Кнопка **📢** у каждого — повторная рассылка; затем выберите все группы или мастер‑группу (обложка с сайта, продающий текст).
...
- **Дедуп апдейтов** — in-memory в одном serverless-инстансе; публикация защищена атомарным захватом черновика, а подтверждение рассылки поста — атомарным захватом сессии в Supabase.
