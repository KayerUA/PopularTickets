...
export async function rewriteBroadcastWithGemini(source: string, instruction: string): Promise<string> {
  ...
  const prompt = [
    ...
    "Строго сохрани все факты из исходника: даты, время, цены, валюты, адреса, ссылки, названия, имена, хэштеги и CTA. Не придумывай факты.",
    "Если инструкция просит сократить — убери воду, но не обязательные факты. Язык оставь как в исходнике, если явно не сказано иначе.",
  ...
  for (const model of models) {
    ...
    if (response.ok && text) return text.slice(0, 4096);
