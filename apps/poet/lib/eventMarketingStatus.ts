...
export function resolveEventMarketingStatus(input: {
  ...
  if (input.remaining <= 0) return "sold_out";

  const threshold = Math.max(LAST_TICKETS_MIN_ABS, Math.ceil(input.totalTickets * LAST_TICKETS_PCT));
  if (input.remaining <= threshold) return "last_tickets";

  const msUntil = start.getTime() - now.getTime();
  if (msUntil <= STARTING_SOON_HOURS * MS_HOUR) return "starting_soon";
  if (msUntil <= THIS_WEEK_DAYS * MS_DAY) return "this_week";

  return null;
}

export function sortEventsForMarketing<T extends { starts_at: string | null }>(events: T[]): T[] {
  const now = Date.now();
  const future: T[] = [];
  const past: T[] = [];
  ...
  future.sort((a, b) => new Date(a.starts_at ?? 0).getTime() - new Date(b.starts_at ?? 0).getTime());
