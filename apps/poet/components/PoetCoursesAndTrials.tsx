...
import type { PoetTrialSlotRow } from "@/lib/poetTrials";
...
export async function PoetCourseShowcase() {
...
export async function PoetTrialsAndFlow({ locale, trials }: { locale: AppLocale; trials: PoetTrialSlotRow[] }) {
  const t = await getTranslations("Poet");
            ...
            {trials.map((slot) => {
              ...
              return (
                ...
                >
                  ...
                  {tickets ? (
                    <a
                      href={ticketsEventPage(locale, slot.tickets_checkout_event_slug)}
