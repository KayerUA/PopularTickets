...
Платформа для небольших событий в Польше: афиша на **польском** и **украинском** (`/pl`, `/uk`), оплата **Przelewy24**, билеты с **QR**, **Resend**, админка и **check-in**.
...
PopularTickets/
├── app/
│   ├── [locale]/(site)/        # Публичный сайт: /pl/..., /uk/...
│   │   ├── layout.tsx
│   │   ├── page.tsx
...
- Публичные страницы: **`/pl/...`** и **`/uk/...`**. Корень **`/`** перенаправляется на **`/pl`** (middleware).
...
После деплоя: `/admin/login` → создайте и опубликуйте событие. Реквизиты и блок Przelewy24: **`/pl/firma`** или **`/uk/firma`**.
