...
- **Check-in**: в production задайте **`CHECKIN_PASSWORD`**; один раз на `/check-in`, дальше сессия на сканирование. Без него production check-in закрыт.
...
- `CHECKIN_PASSWORD` — пароль контролёра для `/check-in`; в production обязателен.
...
- **Кто и как отмечает вход**: страница **`/check-in`**. Сначала вход по **`CHECKIN_PASSWORD`**, затем скан QR / отметка. В админке **`/admin/orders`** — только просмотр.
