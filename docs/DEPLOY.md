...
| 2c | SQL Editor | Если события на сайте «падают» с ошибкой про **`maps_url`**, выполните **`supabase/add-maps-url.sql`** (или полный актуальный `schema.sql` — там есть `add column if not exists maps_url`). |
